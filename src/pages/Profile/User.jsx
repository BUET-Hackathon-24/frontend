...
import { MoveIcon, User2Icon } from 'lucide-react'
...
const User = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await getApiResponse('profileInfo', 'GET')
    ...
    <div className="m-1">
      ...
      <div className="p-1">
        <div className="flex flex-row w-full mb-3 items-center justify-evenly">
          <IconGrid4x4 size={35} className={cn(activeTab == 1 ? 'text-bold' : 'text-gray-700')} />
          <MoveIcon size={35} className={cn(activeTab == 2 ? 'text-bold' : 'text-gray-700')} />
          <User2Icon size={40} className={cn(activeTab == 3 ? 'text-bold' : 'text-gray-700')} />
        </div>

        <div className="grid grid-cols-3 gap-1">
          <img src={user.dp} alt="avatar" className="w-32  aspect-square " />
          <img src={user.dp} alt="avatar" className="w-32 h-32  " />
          <img src={user.dp} alt="avatar" className="w-32 h-32  " />
          <img src={user.dp} alt="avatar" className="w-32 h-32  " />
          <img src={user.dp} alt="avatar" className="w-32 h-32  " />
          <img src={user.dp} alt="avatar" className="w-32 h-32  " />
